...
if ('GET' == $request_method) {
    ...
    if (CSAJAX_DEBUG) {
        foreach($array as $key=>$value) {
           csajax_debug_message("$key holds $value");
        }
    }
...
if (isset($_REQUEST['csurl'])) {
    ...
    if (CSAJAX_DEBUG) {
        csajax_debug_message("raw csurl" . $_REQUEST['csurl']);
        csajax_debug_message("decoded csurl" . $request_url);
    }
...
$curl_cmd = "curl -H 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8' -H 'Accept-Encoding: gzip, deflate' -H 'Accept-Language: en-US,en;q=0.5' -H 'Connection: keep-alive' -H 'Content-Type: application/x-www-form-urlencoded' -H 'Host: www.sgx.com' -H 'Referer: http://www.sgx.com/wps/portal/sgxweb/home/company_disclosure/company_announcements' -H 'User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64; rv:49.0) Gecko/20100101 Firefox/49.0' -H 'X-Requested-With: XMLHttpRequest' " . $request_url . " | gzip -d";
$response_content = shell_exec($curl_cmd);
// finally, output the content
if (preg_match('/\{"(\w+)":(\d+), "items":\[ (.+) \{\}\]\}/', $response_content, $matches)) {
    print('{"' . $matches[1] . '":' . $matches[2] . ', "items": "' . $matches[3] . '" }');
} else {
    print(preg_replace('/,\{\}\]\}$/', ']}', preg_replace('/^\{\}&&/', '', $response_content)));
}
//print($curl_cmd);
